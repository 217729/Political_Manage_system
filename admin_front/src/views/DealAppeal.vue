        ...
        <el-container>
            ...
            <el-aside width="200px" style="background-color: whitesmoke;">
                ...
                <div class="option" style="background-color: gainsboro;">
                    <img src="/static/pictures/申诉管理.png">
                </div>
                <div class="option">
            ...
            <!-- 主要内容栏 -->
            <el-main>
                <span style="font-size:25px;">Appeal handling</span>
                <el-divider />
                <el-tabs v-model="activeName" class="demo-tabs">
                    <el-tab-pane label="待处理申诉" name="first">
                        <el-table :data="need_handle"
                            ...
                            <el-table-column prop="vid" label="申诉事件编号" width="140" sortable />
                            <el-table-column prop="thing" label="申诉事件" width="320" />
                            <el-table-column prop="inf" label="申诉信息" width="300" />
                            <el-table-column label="操作" width="160" fixed="right">
                                <template #default="scope">
                                    <el-popconfirm title="Are you sure to pass this?" @confirm="agree_appeal(scope.row)">
                                        <template #reference>
                                            <el-button link type="primary" size="small">通过申诉</el-button>
                                        </template>
                                    </el-popconfirm>
                                    <el-popconfirm title="Are you sure to reject this?" @confirm="retort_appeal(scope.row)">
                                        <template #reference>
                                            <el-button link type="primary" size="small">驳回申诉</el-button>
                                        </template>
                                    </el-popconfirm>
                                </template>
                            </el-table-column>
                    ...
                    <el-tab-pane label="已处理申诉" name="second">
                        <el-table :data="handled"
                            :height="handled.length - 10 <= 0 ? 'handled.length*10' : '480'"
                            style="width: 1080px;left:50px;" :border="true" :stripe="true">
                            <el-table-column prop="mid" label="商家编号" width="110" sortable></el-table-column>
                            <el-table-column prop="realname" label="商户名" width="310" />
                            <el-table-column prop="vid" label="申诉事件编号" width="140" sortable />
...
export default {
    ...
    async created() {
        const res1 = await request.get(`/need_handle_appeal`)
        console.log(res1.data)
        this.need_handle = res1.data.data
        const res2 = await request.get(`/handled_appeal`)
        console.log(res2.data)
        this.handled = res2.data.data
    },
    methods: {
        async agree_appeal(row) {
            const res = await request.post('/handleit', { vid: toRaw(row).vid, succ: 1 })
        ...
        async retort_appeal(row) {
            const res = await request.post('/handleit', { vid: toRaw(row).vid, succ: 0 })
            console.log(res.data)
            ElMessage({
                message: "申诉驳回成功",
                type: 'success',
